...
var InventoryPanel = ( function (){
	...
	var _CreateCatagoryBtns = function( aCategories )
	{
		...
		for ( var i = 0; i < aCategories.length; i++ ) 
		{
			...
			if ( !elBtn )
			{
				elBtn = $.CreatePanelWithProperties( 'RadioButton', elPanel, aCategories[ i ],
					{
				...
				$.CreatePanelWithProperties( 'Label', elBtn, '', {
	...
	var _CreateInventoryContentPanel = function () {
		return $.CreatePanelWithProperties('Panel', _m_elInventoryMain, 'InventoryMenuContent', {
	...
	var _CreateSubmenusAndListerPanelsForEachCategory = function( aCategories, elParent )
	{
		aCategories.forEach( function( tag, i )
		{
			if ( tag )
			{
				...
				var elCategory = $.CreatePanelWithProperties( 'Panel', elParent, tag, {
					class: 'inv-category'
				} );
	...
	var _CreateNavBar = function (idForNavBar, elParent) {
		var elNavBar = $.CreatePanelWithProperties('Panel', elParent, idForNavBar + '-NavBarParent', {
			class: 'content-navbar__tabs content-navbar__tabs--dark content-navbar__tabs--noflow'
		});


		var elNavBarButtonsContainer = $.CreatePanelWithProperties('Panel', elNavBar, idForNavBar + '-NavBar', {
	...
	var _MakeNavBarButtons = function( elNavBar, listOfTags, onActivate )
	{
		...
		{
			var elButton = $.CreatePanelWithProperties( 'RadioButton', elNavBar, tag + 'Btn', {
				group: groupName,
			...
			if ( nameToken )
			{
				$.CreatePanelWithProperties( 'Label', elButton, '', {
					text: '#' + nameToken
				} );
			}
			else
			{
				...
				if ( icon ) 
				{
					...
					$.CreatePanelWithProperties( 'Image', elButton, '', {
	...
	var _AddSortDropdownToNavBar = function( elNavBar )
	{
		...
		if ( !elDropdown )
		{
			...
			for (var i = 0; i < count; i++) 
			{
				...
				var newEntry = $.CreatePanelWithProperties('Label', elDropdownParent, sort, {
					class: 'DropDownMenu'
				});
	...
	var _SetUpCasketPopup = function( capability, elList )
	{
		...
		if ( capability === "casketstore" || capability === "casketretrieve" )
		{
			...
			if ( !elActionBar )
			{
				elActionBar = $.CreatePanelWithProperties( 'Panel', _m_elSelectItemForCapabilityPopup, 'CapabilityPopupActionBar', 
